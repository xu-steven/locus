...
public class LeveledSiteConfiguration extends SiteConfiguration {
    ...
    public LeveledSiteConfiguration(Integer minimumCenterCount, Integer maximumCenterCount, List<Integer> potentialSites, SearchSpace searchParameters, Integer taskCount, ExecutorService executor) {
        ...
        for (int i = 0; i < searchParameters.getHigherCenterLevels(); ++i) {
            List<Object> initialHigherLevelCostAndPositions = initialCost(higherLevelSitesArray.get(i), searchParameters.getMinimumCasesByLevel().get(i + 1), searchParameters.getCaseCountByOrigin(), searchParameters.getGraphArray(), taskCount, searchParameters.getPartitionedOrigins(), executor);
    ...
    public LeveledSiteConfiguration removeLowestLevelSite(SearchSpace searchParameters, Integer taskCount, ExecutorService executor) {
        ...
        //Compute new parameters
        List<Object> updatedResult = removeSiteCost(newSites, removalPosition, minimumPositionsByOrigin, searchParameters.getMinimumCases(), searchParameters.getCaseCountByOrigin(), searchParameters.getGraphArray(), taskCount, searchParameters.getPartitionedOrigins(), executor);
        double newCost = (double) updatedResult.get(0) * searchParameters.getServicedProportionByLevel().get(0);
